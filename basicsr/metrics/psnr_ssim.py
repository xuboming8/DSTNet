...
@METRIC_REGISTRY.register()
def calculate_psnr_real(img1, img2):
    img1 = img1.astype(np.float32) / 255.0
    img2 = img2.astype(np.float32) / 255.0

    prd_img, tar_img, image_mask, shift = image_align(img1, img2)
    data_range = 1
    err = np.sum((tar_img - prd_img) ** 2, dtype=np.float64) / np.sum(image_mask)
    return 10 * np.log10((data_range ** 2) / err)


@METRIC_REGISTRY.register()
def calculate_ssim_real(img1, img2):
    img1 = img1.astype(np.float32) / 255.0
    img2 = img2.astype(np.float32) / 255.0
    prd_img, tar_img, cr1, shift = image_align(img1, img2)
